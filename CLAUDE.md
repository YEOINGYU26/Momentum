...
### LineRole enum (`mobile/lib/models/chart_line_info.dart`)
```dart
...
|----|------|------|
| `none` | 추세선 | white70 |
| `buyTrigger` | 매수선 | #26A69A (틸) |
| `sellTrigger` | 매도선 | #EF5350 (레드) |
| `alert` | 알림선 | #FFD700 (골드) |
| `stopLoss` | 손절선 | #FF8A65 (오렌지) |
| `takeProfit` | 목표가선 | #4FC3F7 (스카이블루) |

- `ChartLine.role: LineRole` 필드 추가 — 선택된 선에 플로팅 툴바에서 역할 지정
- `_drawLines()` Painter에서 선 엔드포인트 근처에 역할 라벨 배지 렌더링
- **툴바 버튼**: 차트에 "매매선" 버튼 추가 → 역할별 선 일괄 생성/지정 가능
- **자동매매 화면**: 매수 탭 → `buyTrigger` 라인만, 매도 탭 → `sellTrigger` 라인만 필터링

## ChartProvider 티커별 라인 저장 (KAN-18, 완료)

```dart
// Map<ticker, 라인 목록> — 세션 중 티커 전환해도 라인 유지
...
- 티커 전환 시 기존 `_lines = []` 초기화 문제 방지
- `CandleChart(key: ValueKey(ticker), initialLines: savedLines)` 패턴으로 복원
- `initState()` 에서 `initialLines`를 `_lines`에 복사 → 위젯 재생성 시 라인 유지
- SharedPreferences 영구 저장은 KAN-19로 별도 처리 예정

## 가격 축 Nice Tick 알고리즘 (KAN-18, 완료)

- 기존: 고정 4단계 나눔 → `23만`, `24만` 형태로 표시
- 현재: `_niceTicks()` + `_fmtAxisAbs()` 조합
  - 줌 레벨에 따라 100원 / 1,000원 / 10,000원 등 자동 선택
  - 전체 숫자 표기: `230,000` / `231,100` (만단위 약어 X)
- `_niceTicks(lo, hi, target)`: rawStep → magnitude(10의 거듭제곱) → 1/2/5/10 배수 반올림

## KIS 호가 단위 (tick size) (KAN-18, 완료)

`app/services/price_monitor.py` — `_tick_unit(price)` 함수
...
- 추세선 `effective_target()` 반환값을 호가 단위로 반올림 후 주문
- `order_price = round(target / tick) * tick`

## 크로스헤어 미래 이동 수정 (KAN-18, 완료)
